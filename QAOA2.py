...
KAPPA = np.array([2500, 4000, 3000, 2000])
ALPHA = np.array([0.30, 0.25, 0.05, 0.20])
BETA  = np.array([0.10, 0.20, 0.40, 0.15])
FRIC  = np.array([0.15, 0.20, 0.10, 0.25])

# Pairwise interactions coerenti con 4 controlli
INTERACTIONS = {
    (0, 1): (0.05, 0.03),
    (1, 3): (0.04, 0.02)
}

# Dipendenze corrette (indici 0–3)
DEPENDENCIES = [
    (3, 2)   # Patch_Mgmt richiede Offline_Backup
]
...
Q = build_qubo_matrix(company, M=3, normalize=False, Delta=25000)

n_controls = N_CONTROLS
min_active = math.ceil(n_controls / 2 - 1)

qp = build_qp_from_Q(Q, n_controls, min_active=min_active)

converter = QuadraticProgramToQubo(penalty=1e7)
